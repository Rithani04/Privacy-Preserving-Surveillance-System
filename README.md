...
A privacy-preserving computer vision system that detects, tracks, and anonymizes faces in video footage using deep learning. The system identifies a **target person** (the one to keep visible) and replaces all **non-target faces** in every frame with a donor face using AI-powered face swapping — ensuring bystanders and other individuals are anonymized automatically.
